...
public class webconnector {
    ...
    public void selectLowPriceProductfElement() throws Exception {
   List<WebElement> price = driver.findElements(By.xpath("//tbody[@class='wishlist-items-wrapper']/tr/td/span[@class='woocommerce-Price-amount amount']"));
        ArrayList<String> prices=new ArrayList<String>();
        for(int i=0;i<price.size();i++){
            String priceInt = price.get(i).getText().replaceAll("[^0-9.]", "");
            prices.add(priceInt);
        }
        String minPrice = Collections.min(prices);
       driver.findElement(By.xpath("((//tbody[@class='wishlist-items-wrapper']/tr/td/span/bdi[text()='"+minPrice+"'])/../.)/../../td[@class='product-add-to-cart']")).click();
    ...
    public void PerformActionOnElement(String WebElement, String Action, String Text) throws Exception {
        ...
        switch (Action) {
            ...
            case "ElementNotDisplayed":
                ...
                break;
            case "SelectByIndex":
                Select indx = new Select(FindAnElement(WebElement));
                indx.selectByIndex(Integer.parseInt(Text));
                break;
            default:
